def find_player_pos(matrix, player):
...
current_player_position = find_player_pos(matrix, 'w')
other_player_position = find_player_pos(matrix, 'b')
...
while not is_queen and not is_captured:
    ...
    elif current_player_row not in (0, 8 - 1):
        is_queen = True
    else:
...
if is_captured:
    ...
    print(f"Game over! {player} win, capture on {f'{column_name}{row_name}'}.")
if is_queen:
    print(f"Game over! {player} pawn is promoted to a queen at {f'{column_name}{row_name}'}.")
